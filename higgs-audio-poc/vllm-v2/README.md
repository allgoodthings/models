# Higgs Audio v2 - vLLM High-Throughput Setup (V2)

> **Note:** This is v2 of the vLLM setup, created to fix issues with the official `bosonai/higgs-audio-vllm` Docker image (broken tokenizer, etc.). See `../vllm/` for the original (non-production) version.

High-throughput TTS inference using vLLM's continuous batching for concurrent request handling.

## Known Issues (To Fix)

- [ ] Tokenizer initialization fails with official Docker image
- [ ] Audio output format compatibility issues
- [ ] Model loading errors in some configurations
